 ...
 * ENFORCEMENT: 99% Precision | SMC, ICT, WYCKOFF, DXY
 */
...
async function executeSurgicalScan() {
    ...
    if (files.filter(f => f).length < 2) { alert("SYSTEM ERROR: 15M + 1M required for Precision Scan."); return; }
    
    btn.disabled = true; btn.style.opacity = "0.5"; btn.innerText = "SURGICAL SCANNING...";
    try {
        ...
        btn.disabled = false; btn.style.opacity = "1"; btn.innerText = "EXECUTE COMMAND"; 
    }
}
...
async function fetchDualModeAnalysis(key, images, isDay) {
    ...
    // PASS 1: OCR DATA INTEGRITY (Temperature 0.0)
    const p1 = { contents: [{ parts: [{ text: "Extract exactly from charts: Asset Ticker, Live Price, 1H Bias, 15M Structure, 1M Sweep status. RETURN JSON ONLY." }, ...inlineData] }], generationConfig: { response_mime_type: "application/json", temperature: 0.0 } };
    ...
    const p2 = { contents: [{ parts: [{ text: `Strategy: 8-Core Matrix (SMC/ICT/Wyckoff/DXY). Facts: ${JSON.stringify(facts)}. ${modeDirectives} Apply 0.05% SL Buffer. Return JSON: {bias, entry, sl, tp, logic, conf, strategy, scout: {level, msg}}` }] }], generationConfig: { response_mime_type: "application/json", temperature: 0.1 } };
    ...
    // MATHEMATICAL RR GUARDRAILS
    const minRR = isDay ? 3.0 : 1.5; const maxRR = isDay ? 8.0 : 2.5;
    ...
    sig.assetType = facts.assetType; return sig;
}
...
function renderOutput(data, isDay) {
    ...
    let scout = (data.bias === 'WATCHING' && data.scout) ? `<div class="mt-4 p-4 border border-cyan-500/30 bg-cyan-500/5 rounded-2xl border-dashed"><p class="text-[10px] font-black text-cyan-400 uppercase mb-1">Scout POI: ${data.scout.level}</p><p class="text-[12px] text-white italic">"${data.scout.msg}"</p></div>` : '';

    document.getElementById('logicSummary').innerHTML = `
        <div class="flex gap-2 mb-3">
            <span class="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-[9px] font-black uppercase">${data.strategy}</span>
        ...
        <p class="text-white/80 font-bold">${data.logic}</p>${scout}`;

    const bal = parseFloat(localStorage.getItem('omni_bIn')) || 0;
    const rsk = parseFloat(localStorage.getItem('omni_rIn')) || 0;
    if (bal && rsk && risk > 0) document.getElementById('lotVal').innerText = ((bal * (rsk/100)) / (risk * (ASSET_SPECS[data.assetType]?.lotDivisor || 1))).toFixed(4);
}
